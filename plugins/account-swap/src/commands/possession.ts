...
import { cleanupPossession, pendingPossessions } from "../utils/cleanup";
            ...
            try {
                ...
                if (!currentToken) {
                    return sendBotMessage(
                        ctx.channel.id,
                        "❌ **Possession Invite Failed**: Unable to retrieve your account token. Please try reloading Discord.",
                    );
                }
                ...
                sendBotMessage(
                    ctx.channel.id,
                    `Possession invite sent to <@${otherUser.id}>. They now have access to your account.`,
                );
            } catch (err) {
                console.error("Error in possess-invite command:", err);
                sendBotMessage(
                    ctx.channel.id,
                    `❌ **Possession Invite Failed**: ${err.stack ?? err.message ?? "Unknown error occurred"}.`,
                );
            }
        },
    });
        ...
        async execute(args, ctx) {
            try {
                ...
                if (pendingPossessions.has(otherUser.id)) {
                    return sendBotMessage(
                        ctx.channel.id,
                        "You already have a pending possession request with this user.",
                    );
                }
                ...
                sendBotMessage(
                    ctx.channel.id,
                    `❌ **Possession Request Failed**: ${err.stack ?? err.message ?? "Unknown error occurred"}.`,
                );
            }
        },
    });
